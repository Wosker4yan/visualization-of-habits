...
def MultiplePlots(xValues, allYValues):
    ...
    for yValues in allYValues:
        plt.plot(list(map(str, xValues)), list( map(float, yValues) ), label = "file" + str(i))
...
def GetCoordinates( n , separator , fExtension ):
    ...
    for i in range(n):
        coordinates.append( FillList( ReadFile("C:/Users/wosker4yan/Desktop/Work and Study/sendtex/Python_tutorials/matplotlib_habits_visualization/workouts.csv" + str(i+1) + fExtension), separator ) )
    return coordinates

def ReadFile(path):
    'Function to read CSV file and store file data rows in list.'
    try:
        fileCSV = open("C:/Users/wosker4yan/Desktop/Work and Study/sendtex/Python_tutorials/matplotlib_habits_visualization/faps.csv","r") #Opens file
